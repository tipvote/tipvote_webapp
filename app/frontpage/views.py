...
@frontpage.route('/tipped', methods=['GET'])
@login_required
def frontpage_coinposts():
    ...
    posts = posts.filter(CommonsPost.hidden == 0)
    posts = posts.filter(or_(CommonsPost.age == post_18, CommonsPost.age == allpost))
    posts = posts.filter(or_(CommonsPost.total_recieved_btc_usd > 0,
                             CommonsPost.total_recieved_promotion_btc_usd > 0)
                         )
    ...
    next_url = url_for('frontpage.frontpage_coinposts', page=posts.next_num) \
        if posts.has_next else None
    prev_url = url_for('frontpage.frontpage_coinposts', page=posts.prev_num) \
        if posts.has_prev else None
